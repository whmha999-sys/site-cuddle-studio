...
BEGIN
  ...
  -- Recompute subtotal in BASE currency from products table
  FOR v_item IN SELECT * FROM jsonb_array_elements(v_items)
  LOOP
    v_pid := v_item->>'id';
    v_qty := greatest(1, coalesce((v_item->>'qty')::int, 1));
    SELECT price INTO v_price FROM public.products WHERE id = v_pid AND active = true;
  ...
  v_tax_base := round((v_subtotal_base - v_discount_base) * 0.10, 2);
  v_ship_base := CASE WHEN v_subtotal_base > 100 THEN 0 ELSE 3 END;
  v_total_base := v_subtotal_base - v_discount_base + v_tax_base + v_ship_base;

  v_subtotal := round(v_subtotal_base * v_rate, 2);
  v_discount := round(v_discount_base * v_rate, 2);
  v_tax := round(v_tax_base * v_rate, 2);
